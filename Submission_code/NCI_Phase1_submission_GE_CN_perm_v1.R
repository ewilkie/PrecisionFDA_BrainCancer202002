...
### On standardised and unstandardised data ##
##############################################
...
exp_filt <- exp[,which(colnames(exp) %in% as.vector(hu$Input))]

## combine
indata <- cbind(exp_filt,x)
...
saveRDS(varlrds, "../Output/Varlsel_GE_CN.rds")

#write.table(data, "../Output/Filtered_Input_data_Sc3_GE_CN.txt", sep="\t", row.names=F, quote=F)

## 5 rnadom permutations
...
for(i in 1:3){
  ...
  outf <- paste("../Plots/SC3_AUC_perm",i, "_v2.pdf", sep="")
  ggsave(outf, gp)

  saveRDS(rds_out, file = "../Output/Fits_GE_CN_permutation_v2.rds")
}
