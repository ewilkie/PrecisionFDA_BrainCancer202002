...
gbox <- ggplot(unexpm, aes(variable, value)) + geom_boxplot(position="dodge", outlier.size=.2)
print(gbox)
```
...
## order by mean
unexpo <- sig[,order(apply(sig, 2 , mean))]
## format for plottion
unexpm <- melt(unexpo)
...
pdfs <- rbind(pd_gbm,pd_knn,pd_nb)
...
ggplot(pdfs, aes(x=specificities,y=sensitivities,colour=method)) + geom_point(size=.5) + geom_line(size=1.15) + scale_x_continuous(trans = "reverse") + ggtitle("AUC - Comparisons on Stochastic Gradient Boosting (GMB), K-nearest neighbours (KNN) and Naive Bayes (NB)")
```
